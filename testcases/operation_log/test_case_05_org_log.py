...
class TestCase05OrgLog(unittest.TestCase):
    # 公司部门管理记录
    def setUp(self):
    ...
    def test_org_log(self):
        ...
        # 点击公司部门管理记录
        self.operation_log.click_org_log()




        csv_file = self.operation_log_read_csv.read_csv('org_log.csv')
        ...
        for row in csv_data:
            ...
            # 输入时间段
            self.operation_log.org_input_time(org_log['start_time'],org_log['end_time'])

            # 选择类型
            self.operation_log.org_choose_type(org_log['type'])

            # 点击搜索
            self.operation_log.org_click_search()

            '''# 获取搜索结果
            result_info = self.operation_log.get_org_log_search_result()
            print(result_info)


            # 获取数据库查询结果
            info = self.operation_log.get_org_log_search_result_by_sql(log_in_account,account_log['start_time'],account_log['end_time'])
            ...
            # 点击重置
            self.operation_log.org_click_reset()
