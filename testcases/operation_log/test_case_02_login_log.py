...
class TestCase02LoginLog(unittest.TestCase):
    # 登录记录
    def setUp(self):
    ...
    def test_login_log(self):
        ...
        # 点击登录记录
        self.operation_log.click_login_log()




        csv_file = self.operation_log_read_csv.read_csv('login_log.csv')
        ...
        for row in csv_data:
            ...
            # 选择时间
            self.operation_log.login_choose_time(login_log['time'])

            # 输入时间段
            self.operation_log.login_input_time(login_log['start_time'],login_log['end_time'])

            # 输入登录账号
            self.operation_log.login_input_account(login_log['account'])

            # 点击搜索
            self.operation_log.login_click_search()

            '''# 获取搜索结果
            result_info = self.operation_log.get_login_log_search_result()
            print(result_info)


            # 获取数据库查询结果
            info = self.operation_log.get_credit_log_search_result_by_sql(log_in_account,login_log['start_time'],login_log['end_time'])
            ...
            # 点击重置
            self.operation_log.login_click_reset()
